...
| REQ-016 | When a turn ends with an uncommitted `decision` or `milestone` entry in a session's `log.md`, the commit guard shall prevent the turn from ending until that entry and its artifact changes are committed. *(Added after the Define milestone, 2026-09-24. See the log.)* | Given a new uncommitted decision entry, when Claude tries to end its turn, then it's blocked with a reason. After the commit, the turn ends. After 2 blocks in a row, the stop is allowed (loop guard). |

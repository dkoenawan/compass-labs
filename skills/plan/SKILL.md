...
**Create the session file immediately.** Derive a kebab-case feature slug from the answer and create `docs/sessions/{today}-{feature-name}/overview.md` right away (create the directory if needed), using today's date in `YYYY-MM-DD` format. Populate the frontmatter and `## Summary` from this raw answer:
...
{raw answer, tightened to 2-3 sentences}
```

This file is the running record of the session from here on — every later phase (Q2, Q3, Discover, Layer design, Synthesis) edits it in place rather than holding content in conversation only. The file exists whether or not the session reaches approval; an abandoned or rethought session leaves an honest partial draft instead of nothing on disk.
...
Synthesize all answers into a complete feature spec using the structure from [template.md](template.md), editing the session file created in Q1 in place — do not create a new file.
...
> This is the complete feature spec, already saved to disk as a draft. Before I mark it approved, I want to make sure everything is right. Three options:
>
> 1. **This nails it** — mark the spec approved
...
- **multiSelect**: false
- **Options**:
  - **"This nails it"** — description: "Spec is accurate and complete — mark it approved"
...
**If "Adjust"**: Ask what to change, revise the relevant sections in the session file, present again. Repeat until approved.
**If "Rethink"**: Ask which phase(s) to revisit, redo those questions, then re-synthesize (still editing the same session file).
...
The spec file was created in Q1 and edited throughout every phase since. There is nothing left to create — finalize it:

1. Confirm `docs/sessions/{date}-{feature-name}/overview.md` contains the full [template.md](template.md) structure with all synthesized content (fill in any section not yet written during synthesis)
2. Flip the status line from `Status: Draft` to `Status: Approved`
...
After finalizing the file and writing registry stubs, present:

1. **File finalized** — the full path to the spec file (created at Q1, approved as of this phase)
...
>
> - [ ] The spec file exists at `docs/sessions/{date}-{feature-name}/overview.md` on disk (created at Q1, not deferred to Phase 5)
> - [ ] The file contains all sections from `template.md`, filled with real content (no placeholder text)
> - [ ] The file's status line reads `Status: Approved`
